...
jmx_options() {
  if [ -n "${JMX_PORT:-}" ]; then
    echo "-javaagent:./jmx/jmx_prometheus_javaagent.jar=${JMX_PORT}:./jmx/jmx_config.yml"
  fi
}
...
java_options() {
  ...
  # of this awk idiom
  echo "${JAVA_OPTIONS:-} ${JAVA_OPTS:-} $(run_java_options) $(debug_options) $(jmx_options) $(proxy_options) $(java_default_options) -Dfile.encoding=UTF8 " | awk '$1=$1'
}
